			...
			<ul class="post-type-settings">
				<li>
					<div class="row">
						...
						<div class="field">
							<label><input type="checkbox" name="nestedpages_posttypes[<?php echo esc_attr($type->name); ?>][replace_menu]" value="true" <?php if ( $type->replace_menu ) echo 'checked'; ?> /><?php echo __('Replace Default') . ' ' . $type->label . ' ' . __('Menu', 'wp-nested-pages'); ?></label>
						</div><!-- .field -->
					</div><!-- .row -->
				</li>
				<li>
				...
				<li>
					<div class="row">
						...
						<div class="field">
							<label><input type="checkbox" name="nestedpages_posttypes[<?php echo $type->name; ?>][disable_nesting]" value="true" <?php if ( $type->disable_nesting ) echo 'checked '; ?>/><?php echo __('Disable Nesting for', 'wp-nested-pages') . ' ' . esc_html($type->label); ?></label>
						</div>
					</div><!-- .row -->
				</li>
				<?php endif; ?>
				<li>
					...
					<div class="row">
						...
						<div class="field">
							<label><input type="checkbox" name="nestedpages_posttypes[<?php echo $type->name; ?>][thumbnails][display]" value="true" <?php if ( $thumbnails_enabled ) echo 'checked'; ?> data-enable-thumbnails /><?php echo __('Display Thumbnails for', 'wp-nested-pages') . ' ' . esc_html($type->label); ?></label>
									...
									<ul data-columns-sortable>
										<?php
											...
											foreach ( $selected_columns as $name => $label ) :
												$out .= '<li>';
												$out .= '<label>';
												$out .= '<input type="checkbox" name="nestedpages_posttypes[' . esc_attr($type->name) . '][columns][' . esc_attr($name) . ']" value="' . esc_attr($label) . '"';
											...
											foreach ( $all_columns as $name => $label ) :
												...
												$out .= '<li>';
												$out .= '<label>';
												$out .= '<input type="checkbox" name="nestedpages_posttypes[' . esc_attr($type->name) . '][columns][' . esc_attr($name) . ']" value="' . esc_attr($label) . '" />' . esc_html($label);
												$out .= '</label>';
												$out .= '</li>';
											endforeach;
											echo $out;
				...
				<li>
					<div class="row">
						...
						<div class="field">
							<label><input type="checkbox" data-toggle-nestedpages-sf-settings name="nestedpages_posttypes[<?php echo esc_attr($type->name); ?>][standard_fields_enabled]" value="true" <?php if ( $type->standard_fields_enabled ) echo 'checked '; ?>/><?php _e('Configure Standard Fields', 'wp-nested-pages'); ?></label>
							...
							<?php
								...
								if ( $acf_fields ) :
									...
									foreach ($acf_fields as $field){
										...
										$out .= '/>' . $field['label'] . ' (' . esc_html($field['type']) . ')';
										$out .= '</label>';
										$out .= '</li>';
									}
									$out .= '</ul>';
